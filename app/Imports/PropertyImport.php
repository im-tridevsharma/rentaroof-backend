...
class PropertyImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row) {
            ...
            $property->posting_as = $row['posting_as'] ?? '';
            $property->bedrooms = $row['bedrooms'] ?? 1;
            $property->balconies = $row['balconies'] ?? 1;
            $property->floors = $row['floors'] ?? 0;
            $property->furnished_status = $row['furnished_status'] ?? '';
            $property->ownership_type = $row['ownership_type'] ?? 'single';
            $property->bathrooms = $row['bathrooms'] ?? 1;
            $property->carpet_area = $row['carpet_area'] ?? '';
            $property->carpet_area_unit = $row['carpet_area_unit'] ?? 'sqft';
            $property->super_area = $row['super_area'] ?? '';
            $property->super_area_unit = $row['super_area_unit'] ?? 'sqft';
            $property->available_from = $row['available_from'] ? date('Y-m-d', strtotime($row['available_from'])) : date('Y-m-d');
            ...
            $property->maintenence_duration = $row['maintenence_duration'] ?? 'monthly';
